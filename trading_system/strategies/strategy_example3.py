...
class SQZM_DMI_Strategy(BaseStrategy):
    ...
    def calculate_signals(self) -> Dict[str, Any]:
        ...
        # Signals
        self.data['Sell_Signal'] = (self.data['Direction'].shift(1) == -1) & (self.data['Direction'] == 1)
        self.data['Buy_Signal'] = (self.data['Direction'].shift(1) == 1) & (self.data['Direction'] == -1)
    ...
    def plot(self, *args, **kwargs):
        ...
        apds = [
            ...
            mpf.make_addplot(plot_data['Equity_Curve'], panel=1, color='b', ylabel='Equity Curve')
        ]
        mpf.plot(
            plot_data,
            type='line',
...
if __name__ == "__main__":
    ...
    # Optimize
    params_grid = {
        'ch_ce_period': np.arange(5, 51, 1).tolist(),
        'ch_atr_period': np.arange(5, 51, 1).tolist(),
        'ch_multiplier': [round(x, 1) for x in np.arange(2.0, 4.1, 0.1).tolist()]
    }
